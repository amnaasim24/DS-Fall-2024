...
void split(Node *&root, Node *node)
{
    ...
    for (int i = 0; i < node->keys.size(); i++) cout << node->keys[i] << " ";
    cout << endl;
    ...
    if (!node->isLeaf)
    {
        ...
        for (int i = 0; i < leftChild->children.size(); i++) leftChild->children[i]->parent = leftChild;
        for (int i = 0; i < rightChild->children.size(); i++) rightChild->children[i]->parent = rightChild;
    }
    ...
    else
    {
        ...
        if (parent->keys.size() == 3) split(root, parent);
    }
}
Node *mergeTrees(Node *root1, Node *root2)
{
    ...
    for (int i = 0; i < root1->keys.size(); i++) mergedRoot->keys.push_back(root1->keys[i]);
    mergedRoot->children.push_back(root1);
    root1->parent = mergedRoot;
    for (int i = 0; i < root2->keys.size(); i++) mergedRoot->keys.push_back(root2->keys[i]);
    ...
    if (mergedRoot->keys.size() == 3) split(mergedRoot, mergedRoot);
    return mergedRoot;
}
void printTree(Node *root, int level = 0)
{
    if (!root) return;
    cout << "Level " << level << ": ";
    for (int i = 0; i < root->keys.size(); i++) cout << root->keys[i] << " ";
    cout << endl;
    for (int i = 0; i < root->children.size(); i++)printTree(root->children[i], level + 1);
}
int main()
{
