...
public:
    int data, height;
    Node *left, *right;
    Node(int value): data(value), height(0), left(NULL), right(NULL) {}
};
...
bool isAVLHelper(Node *node, int &height, int minVal = INT_MIN, int maxVal = INT_MAX)
{
    if (node == NULL)
    {
        height = 0;
...
int main()
{
    ...
    if (isAVLTree(root)) cout << "AVL tree." << endl;
    else cout << "Not an AVL tree." << endl;
